# Houshou Marine (宝鐘マリン) — Official Profile & Settings

> Research compiled on 2026-06-21 from official and encyclopedic sources.

---

## Basic Information

| Field | Value |
|---|---|
| **Name** | Houshou Marine (宝鐘マリン) |
| **Nickname** | Senchou (船長 — "Captain") |
| **Birthday** | July 30 |
| **Zodiac** | Leo |
| **Blood Type** | O |
| **Height** | 150 cm |
| **Debut Date** | August 11, 2019 |
| **Generation** | Hololive 3rd Generation ("hololive Fantasy") |
| **Fan Name (JP)** | 宝鐘の一味 (Houshou no Ichimi — "Houshou's Pirate Crew") |
| **Fan Name (EN)** | Houshou Pirates / Tentacrew |
| **Illustrator** | Akasa Ai (あかさあい) — [Twitter](https://twitter.com/AkasaAi) |
| **Live2D Rigging** | Ilie Tou |
| **YouTube Channel** | [Marine Ch. 宝鐘マリン](https://www.youtube.com/@HoushouMarine) |
| **Subscribers** | 4.32 million (as of March 2026) |
| **Hashtags** | Stream: `#マリン航海記` / Fan Art: `#マリンのお宝` / Clips: `#わかるマリン` / Memes: `#宝鐘海賊団クソコラ部` / NSFW: `#沈没後悔日記` / Suggestions: `#マリン船長これやってください` |
| **Oshi Mark** | 🏴‍☠️ |
| **Distinctive Features** | Heterochromia (left iris red, right iris golden) |

---

## Character Design Themes

- **Pirate captain** — Her core identity. She calls herself the Captain (Senchou) of the Houshou Pirates.
- **Treasure hunter** — Loves jewels, treasure, and money; her dream is to go treasure hunting as a real pirate.
- **Cosplayer** — Her official bio states her pirate outfit is just cosplay; she does not actually own a ship and is working as a VTuber to raise funds for one.
- **Erotic femme fatale** — Described as a "sensual, femme fatale boss" who acts like a big sister and seduces people. Canonically "around 17."
- **Otaku persona** — Known for deep knowledge of 90s/00s anime, manga, doujinshi, and internet culture.

---

## Official Lore / Backstory

From the official hololive talent page:

> "Ahoy! Captain of the Houshou Pirates, Houshou Marine here! Keep 'er steady!"
>
> Loves jewels, treasure, and money. Her dream is to become a pirate and go treasure hunting. In order to get a pirate ship, she is now stacking up money as a VTuber on the great lands, just like how the other land-lovers do (which also means she is currently just a girl cosplayer).
>
> She acts like a big sis and sometimes seduces people like a succubus does. A sensual, femme fatale boss, and a young one, at that! (Around 17)
>
> Her pirate outfit is just cosplay, and quite lewd when you see it that way.

**Dream (from official site):**
> "The plan is procure a pirate ship, set sail with you guys, and at the end of our journey remark, 'My greatest treasure was this scenery and the friends we made along the way.'"

**Q&A from official site:**
- **"Are you really 17?"** — Yes. (2021 was her third go-around.)
- **"Are you really a pirate?"** — "Piracy isn't restricted to just the crimes or the travels, you know. Piracy is a way of life. And my heart is always sailing the seas. And so I say, I am a pirate."

**Real-world origin:** Marine became interested in VTubing after watching Kaguya Luna, then became a fan of Nijisanji's Tsukino Mito. She cites Shirakami Fubuki and Minato Aqua as inspirations for auditioning for Hololive.

**Name etymology:** The kanji 宝鐘 (Houshou) mean "treasure" and "bell" respectively. マリン (Marine) is written in katakana; the name as a whole evokes "treasure bell of the sea." Her surname is intentionally homophonous with "Houshou" (宝鐘), which also reads as "Houshou" (a type of bell).

---

## Costume Variations

Based on official site image galleries, Marine has at minimum the following known costume/outfit variants:

1. **Default Pirate Outfit** — Standard captain look with pirate hat, naval coat, and corset.
2. **Gothic Lolita Outfit** — Revealed October 2020; gothic-style dress.
3. **Casual / Modern Outfits** — Multiple casual civilian styles (at least 8 variations shown on the official site gallery thumbnails, including different hairstyles and accessories).
4. **Summer / Swimsuit Variants** — Seasonal swimsuit costumes.
5. **3D Model** — Available for MMD via [nicovideo](https://3d.nicovideo.jp/works/td78500).
6. **Formal / Concert Costumes** — Outfits worn during hololive fes events and her 1st solo live "Ahoy!! You're All Pirates" (Dec 2024).

*Note: Specific costume names and release dates beyond the default and gothic lolita would require further detailed sourcing from the Hololive fan wiki (which was inaccessible during research due to 403 errors).*

---

## Key Milestones

| Date | Milestone |
|---|---|
| Aug 11, 2019 | Debut as last member of Hololive 3rd Gen |
| Oct 27, 2020 | Reached 1M subscribers (3rd in hololive) |
| Sep 18, 2021 | Reached 2M subscribers (5th in hololive) |
| Dec 2023 | First VTuber invited to perform on Fuji TV's FNS Music Festival |
| Jan 10, 2024 | Reached 3.01M subs — most-subscribed JP VTuber, surpassing Kizuna AI |
| Dec 7-8, 2024 | 1st Solo Live "Ahoy!! You're All Pirates" at K-Arena Yokohama |
| May 5, 2025 | Reached 4M subscribers — second VTuber ever to do so (after Gawr Gura) |

---

## Source URLs

- **Hololive Official Site (EN):** https://hololive.hololivepro.com/en/talents/houshou-marine/
- **Hololive Official Site (JP):** https://hololive.hololivepro.com/talents/houshou-marine/
- **Wikipedia:** https://en.wikipedia.org/wiki/Houshou_Marine
- **YouTube Channel:** https://www.youtube.com/@HoushouMarine
- **Hololive Wiki:** https://hololive.wiki/wiki/Houshou_Marine
- **Hololive Fan Wiki (Fandom):** https://hololivevtuber.fandom.com/wiki/Houshou_Marine
