# 角色設定資料：Ayunda Risu（アユンダ・リス）

> 用於同人遊戲開發的完整角色參考資料
> 調查日期：2026-06-21

---

## 一、基本資料

| 項目 | 內容 |
|------|------|
| **名字** | Ayunda Risu（アユンダ・リス） |
| **所屬** | hololive Indonesia 1st Generation（AREA 15） |
| **出道日** | 2020年4月10日 |
| **生日** | 1月15日 |
| **年齡** | 17歲（設定） |
| **身高** | 153 cm |
| **體重** | 42 kg |
| **粉絲名** | Risuners |
| **會員名** | Prisuners（監獄 + Risuner 的合成詞） |
| **代表Emoji** | 🐿️（松鼠） |
| **角色設計** | Yatomi（やとみ）[@8103x](https://twitter.com/8103x) |
| **Live2D建模** | Himono（v1.0）、Brian Tsui（v2.0）、Kurorogi Hitsugi（v2.1/v3.0）、Enma A（v5.0） |
| **3D模型** | 2022年3月20日（hololive 3rd fes. Link Your Wish） |
| **語言** | 印尼語（主要）、英語、日語 |
| **平台** | YouTube / Twitter / Instagram / Facebook / Bilibili |
| **YouTube** | [Ayunda Risu Ch. hololive-ID](https://www.youtube.com/channel/UCOyYb1c43VlX9rc_lT6NKQw) |
| **Twitter** | [@ayunda_risu](https://twitter.com/ayunda_risu) |
| **Bilibili** | ayunda_risu |

---

## 二、角色設定與背景故事

### 官方設定
> 來自魔法森林的松鼠女孩。她在人類世界迷路了，但由於某些原因，她最終住進了一位好心姐姐的家。為了回報這份恩情，她決定成為一名VTuber。

### 背景故事要點
1. **出身**：來自異世界的魔法森林的松鼠少女（異世界穿越題材）
2. **來到人類世界**：迷路而誤入人類世界
3. **恩人**：被一位溫柔的「お姉ちゃん」（Onee-chan）收留
4. **動機**：為了報答恩情而成為VTuber，傳遞歡樂
5. **種族特徵**：松鼠亞人——官方設定有**4隻耳朵**（2個人耳 + 2個松鼠耳在頭頂），尾巴從裙子的洞穿出

### 名字由來
- **Ayunda** → 印尼語「Ayu」（美麗/漂亮）
- **Risu** → 來自樹鼩的學名 *Tupaia splendidula*
- **完整意思**：「漂亮松鼠」

---

## 三、外觀設計與服裝

### 主要服裝（預設）
- **配色**：橘色、棕色、奶油色（松鼠色系）
- **特徵**：
  - 毛茸茸的大松鼠尾巴（從裙子開洞伸出，此為繪師官方設定）
  - 頭頂松鼠耳 + 人類耳朵（共4耳）
  - 橡實與堅果主題裝飾
  - 連帽衫風格的休閒上衣
  - 棕色/白色長襪與鞋子

### 後續服裝
- 第2套：10萬訂閱紀念（2020年9月）—— 休閒連帽設計
- 第3-7套：後續陸續推出
- Wisu服裝：Q版兒童風格
- 多套祭典/活動限定服裝（hololive 4th/6th/7th fes）

---

## 四、個性特質

### 性格核心
- **元氣（Genki）** —— 精力充沛、活潑開朗
- **調皮搗蛋** —— 喜歡對朋友和觀眾搞小惡作劇
- **怕生但容易混熟** —— 對新人害羞，但熟起來後很放得開
- **嚴格的管理員（Warden）** —— 對聊天室秩序要求嚴格
- **愛哭鬼** —— 出道時自述為「clumsy, crybaby」
- **有決心** —— 即使害怕恐怖遊戲也會為了觀眾去玩

### 聲線切換（角色扮演技能）
Risu 最著名的特色之一——能自由切換四種聲線：

| 模式 | 名稱 | 描述 |
|------|------|------|
| **Risu** | 預設 | 高音可愛松鼠聲 |
| **Wisu** | 孩童模式 | 更高音、孩子氣的聲音 |
| **Ayunda** | 魔女模式 | 低沉成熟的聲音 |
| **Wisnu** | 帥哥模式 | 帥氣男性聲音 |

這項技能被稱為「Risu DLC」，觀眾可以透過捐贈點播不同聲線。

### 喜好
- **喜歡**：お姉ちゃん、可愛的東西、帽子、食物、**腰果**（最喜歡的堅果）
- **不喜歡**：蟲子、幽靈、狗（出道時提到）
- **喜歡的Hololive成員**：白上フブキ、夏色まつり、尾丸ポルカ、犬山ころね（尤其是ころね）
- **喜歡的寶可夢**：Slowpoke（呆呆獸）
- **興趣**：槍械模型/氣槍收藏、漫畫、動畫

### 夢想
- 擁有自己的原創歌曲專輯
- 舉辦個人3D演唱會
- 推出自己的娃娃周邊

---

## 五、招牌台詞與文化用語

### 打招呼/告別
| 台詞 | 說明 |
|------|------|
| **"Hewwrroo everyone~!!"** | 招牌問候（誇張發音的Hello） |
| **"Purupuru ganbari Risu~!"** | 自我介紹詞 |
| **"Konrisu"** | こんにちは/こんばんは + Risu 的合成詞 |
| **"Otsurisu~!"** | お疲れ + Risu（告別用語） |
| **"Brrrr"** | 慌張時的松鼠顫音 |

### 社群用語
| 用語 | 意思 |
|------|------|
| **Bonk** | 被踢出聊天室/被處罰 |
| **Cactus** | Risu用來代替髒話的用語（保持清純形象） |
| **Piggy Bank** | 跟Moona有關的梗（見下文軼事） |
| **Warden** | 獄長——因嚴格管理聊天室而得的稱號 |
| **Prisuners** | 監獄（Prison）+ Risuner——會員身分稱呼 |
| **Seiso Degenerate** | 「表面清純內心混沌」——理想粉絲形象 |

### 官方Hashtag
| Hashtag | 用途 |
|---------|------|
| #Risu_Live | 直播討論 |
| #GambaRisu | 粉絲創作 |
| #iRISUtration | 粉絲創作（替代） |
| #Risu_meme | 迷因 |
| #Risuggestion | 內容建議 |
| #CongRISUlation | 慶祝 |
| #Risuners | 粉絲社群 |

---

## 六、重要軼事與成就

### 1. 訂閱里程碑
| 里程碑 | 日期 |
|--------|------|
| 1萬 | 2020年4月11日（出道次日） |
| 10萬 | 2020年8月9日（銀盾） |
| 50萬 | 2021年8月11日（金盾） |
| 80萬 | 2023年3月13日 |
| 90萬 | 2024年11月12日 |
| 100萬 | **2026年6月14日**（鑽石盾） |

### 2. 豬豬存錢筒事件（2020年7月）
Risu 直播數硬幣時，Moona 突然亂入，開玩笑說那些硬幣是從她的豬豬存錢筒偷來的。從此成為一個經典梗——每當 Risu 提到錢就會被提起。

### 3. Non-stop Nut November（NNN）
從2020年開始的年度企劃——不是禁食堅果，而是宣傳堅果（腰果、杏仁等）的營養價值和美味。每年11月固定舉辦，已成為傳統。

### 4. AREA 15 家庭設定
與 Moona Hoshinova 和 Airani Iofifteen 組成的hololive ID一期生團體：
- **Iofifteen** = 爸爸（負責畫圖的藝術家）
- **Moona** = 媽媽（嚴格但溫柔）
- **Risu** = 小孩（精力充沛、調皮的松鼠）
- 每月15日定期聯動，已成為長久傳統

### 5. Hololive Meet 大使
Risu 被選為三位 **Hololive Meet 大使**之一，與 **Tokino Sora** 和 **Gawr Gura** 並列。

### 6. 音樂成就
#### 原創歌曲
| 歌曲 | 發佈日期 |
|------|---------|
| ALiCE&u | 2022年1月16日 |
| 1:15 AM | 2023年4月13日 |
| Sing Out | 2023年10月31日 |
| Dramatic XViltration（AREA15） | 2024年4月15日 |
| HI:STREET TIME | 2024年7月2日 |
| Senandung Jiwa | 2024年8月18日 |
| metropolis | 2025年1月16日 |
| Juara Khatulistiwa（hololive Indonesia） | 2025年4月7日 |
| 染められタイム‼（hololive IDOL PROJECT） | 2025年7月10日 |
| Show Them (Who You Are) | 2025年11月9日 |
| MIR//or | 2025年11月22日 |
| 星が降る / As Star Fall | 2026年1月16日 |

#### 知名翻唱
- **KING**（Kanaria）—— 展示聲域的代表作

### 7. 吉祥物
- **Riscot** —— 由Holostars成員荒咬オガ繪製
- **Risusaurus** —— 同為オガ繪製

### 8. 特殊稱號
- **Tupaisen** —— HoloID二期生對她的愛稱（tupai「松鼠」+ paisen「先輩」的倒裝）

### 9. 重要Live演出
| 活動 | 日期 | 備註 |
|------|------|------|
| hololive 3rd fes. Link Your Wish | 2022年3月20日 | 3D模型首次披露 |
| hololive 5th fes. | 2024年3月16日 | |
| Risu's Isekai Express : Special Performance Delivery | 2025年1月15日 | 首次3D生日個人Live |
| hololive 6th fes. | 2025年3月8日 | |
| ID 5th Anniversary LIVE Chromatic Future | 2025年11月15日 | |
| hololive 7th fes. | 2026年3月7日 | |

### 10. 週年紀念服裝
- **5週年**（2025年4月10日）：「THE EARTH」紀念服裝
- **6週年**（2026年4月10日）：新週年紀念服裝

### 11. 其他軼事
- **ALiCE&u / Alice原名**：Risu曾透露原本想為自己取名Alice，此設定藏於首張原創曲ALiCE&u歌名中
- **Korone與Okayu的女兒**：粉絲圈玩笑設定——Risu是犬山ころね（狗）和貓又おかゆ（貓）的女兒
- **4耳確認**：Q&A中確認角色設定有4隻耳朵（2人耳+2松鼠耳）
- **Bilibili**：擁有Bilibili帳號，面向中文圈粉絲
- **Yagoo的夢想**：Risu和HoloID的成功讓Yagoo感到創立偶像公司的夢想成真
- **TRUST（原Ko-fi）**：接受TRUST平台贊助

---

## 七、遊戲設定建議（同人遊戲用）

### 適合融入遊戲的元素

**角色職業/定位：**
- 森林派來的松鼠使者（魔法/自然系角色）
- 調皮盜賊/陷阱專家（呼應愛惡作劇的設定）
- 聲線切換作為技能機制（四種模式對應不同能力）

**技能點子：**
- **聲線切換**：Risu（可愛妨礙）、Wisu（治癒）、Ayunda（魔法攻擊）、Wisnu（物理攻擊）
- **Bonk**：擊退/暈眩技能
- **Brrr**：範圍驚嚇/迷惑效果
- **Cactus**：無傷害但嘲諷效果
- **堅果投擲**：遠程攻擊（最愛腰果）

**世界觀設定：**
- 主線：從魔法森林誤入人類世界的松鼠少女，尋找回去的方法（或選擇留下）
- NPC：Moona（媽媽型角色）、Iofi（爸爸型角色）、Onee-chan（任務委託人）
- 特殊關卡：Non-stop Nut November 堅果收集活動

**視覺設計要素：**
- 主色調：橘色、棕色、奶油色
- 松鼠耳 + 人耳（4耳設計是重要特徵）
- 大尾巴（穿裙而出）
- 堅果/橡實裝飾
- 連帽衫風格服裝

**語音特色：**
- 四種聲線變化（Risu/Wisu/Ayunda/Wisnu）
- 招牌台詞：「Hewwrroo~」「Konrisu」「Otsurisu~」
- Brrr顫音在特定情境觸發

---

## 資料來源

- [hololive 官方檔案](https://hololive.hololivepro.com/en/talents/ayunda-risu/)
- [hololive Fan Wiki - Ayunda Risu](https://hololive.wiki/wiki/Ayunda_Risu)
- [hololive VTuber Fandom Wiki - Ayunda Risu](https://hololivevtuber.fandom.com/wiki/Ayunda_Risu)
- [Seesaawiki (日文) - Ayunda Risu](https://seesaawiki.jp/hololivetv/d/Ayunda%20Risu)
- [Playboard 分析](https://playboard.co/en/channel/UCOyYb1c43VlX9rc_lT6NKQw)
- [YouTube頻道](https://www.youtube.com/channel/UCOyYb1c43VlX9rc_lT6NKQw)
- 維基百科 - Hololive Production
