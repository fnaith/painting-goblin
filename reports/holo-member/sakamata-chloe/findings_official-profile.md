# Sakamata Chloe (沙花叉クロヱ) — Official Profile Research

> **Note:** Data sourced from official hololive production channels, hololive.wiki (Miraheze), fandom wiki, and seesaawiki.jp.

---

## Generation & Debut

- **Generation:** hololive 6th Generation — **Secret Society holoX**
- **Unit:** holoX (role: fixer/cleaner — the "intern" of the group)
- **Debut Stream:** November 29, 2021
- **Group announcement:** November 26, 2021
- **Fellow members:** La+ Darknesss (leader), Takane Lui (secretary), Hakui Koyori (R&D), Kazama Iroha (guard captain)
- **YouTube channel:** https://www.youtube.com/channel/UCIBY1ollUsauvVi4hW4cumw

---

## Official Lore — "The Secret Society Fixer"

Official Hololive profile description:

> "Chomp, chomp, chooomp!"
>
> An intern who acts as a fixer and cleaner for Secret Society holoX.
> Calm and composed at all times, she carries out her orders without so much as batting an eyelid.
> She loves music and can be found listening to it every chance she gets.
> And while she denies it vehemently, she apparently tends to come across as guarded and not very upfront with her thoughts, only letting people get very occasional glimpses into how she really feels.

Key character notes:
- She is an **intern** within holoX (lowest rank, but integral)
- Her job is **fixer and cleaner** — handling dirty work, disposing of evidence, eliminating obstacles
- Portrayed as calm, composed, and professional on the surface
- Has a hidden/guarded personality — not upfront with her true feelings
- Loves music intensely (composing, lyric-writing, singing, voice training)
- Denies being guarded despite clear evidence otherwise

---

## Character Design Details

- **Illustrator / Character Designer:** パセリ (Parsley / Parsley_F) — https://twitter.com/Parsley_F
- **Live2D Modeler:** 入江燈 (jenny / IrisLight) — https://twitter.com/jennya_IRIS
- **3D Modeler:** ユウコテツ (Yukotetsu)
- **Hair:** Long silver-white hair with blue/teal undertones, styled with two distinctive fin-like protrusions at the back (evoking orca/killer whale dorsal fins)
- **Outfit:** Black-and-white theme — a fitted dark dress/suit with white accents, layered with translucent blue fabric elements suggesting water/aquatic themes
- **Headwear:** A black peaked cap with a holoX emblem (military/agent aesthetic)
- **Accessories:** Black gloves, asymmetrical legwear (one thigh-high, one knee-high), choker with a blue gem, belt with pouches/tools
- **Color scheme:** Black, white, and blue/teal (orca/ killer whale colors)
- **Background color:** #ab0e0c / #cf4c4a (red tones)
- **Outside background color:** #921a1b
- **Height:** 148 cm
- **Fan mark:** 🎣 (fishing rod and fish)

### Design Motifs

| Motif | Expression in Design |
|-------|---------------------|
| **Orca / Killer Whale** | Black-and-white color scheme, fin-like hair protrusions, tail-like lower hair |
| **Secret Agent** | Cap, gloves, fitted tactical outfit, tool pouches |
| **Marine / Ocean** | Blue translucent fabric elements, aquatic color palette |
| **Music** | Often depicted with headphones/earbuds (character loves music) |

---

## Official Stats

| Attribute | Value |
|-----------|-------|
| **Birthday** | May 18 |
| **Height** | 148 cm |
| **Unit** | holoX |
| **Illustrator** | パセリ (Parsley) |
| **Fan Name** | Handlers / Zookeepers (飼育員さん) |
| **Community** | Orcart (オーカート) |
| **Nickname** | Shachiken (シャチけん) |
| **Dream 1** | To hold a solo live concert on a big stage |
| **Dream 2** | To get a statue figure |
| **Dream 3** | To collab with an aquarium |

---

## Catchphrases & Streaming Conventions

| Context | Phrase |
|---------|--------|
| **Start-of-stream greeting** | "Chomp, chomp, chomp! It's time to eat!" |
| **End-of-stream greeting** | "Thanks for the food." |
| **Waiting room fish** | `＜＞＜` (ASCII fish) |
| **Addressing fans** | "Handler." |
| **Profile greeting** | "Chomp, chomp, chooomp!" |

---

## Hashtags

| Purpose | Tag |
|---------|-----|
| Stream tags | `#またまたさかまた` |
| Fan art | `#さかまた飼育日記` |

---

## Likes & Interests

### Liked Foods
- **Mentaiko** (spicy cod roe) — favorite food
- **Shortcakes**
- **Natto**
- **Squid**
- **Tomato**

### Hobbies
- **Music** (composing, writing lyrics, singing, voice training)
- **Karaoke**
- **Listening to music** (favorite artists: Sia, Bruno Mars, Yorushika, ZUTOMAYO)
- **Watching movies**
- **Shopping for accessories**
- **Going to theme parks**
- **Sleep**

### Dislikes
- **Ghosts** (horror/spooky things)

---

## Q&A from Official Profile

**Q: What do you do when you're not streaming?**
A: I sleep!

**Q: What do you like doing the most?**
A: Music! I enjoy all of it, from composing, to writing lyrics, to singing! I've also started voice training too!

**Q: Which stream of yours would you recommend for a first-time viewer?**
A: My 3D reveal stream, 'cuz I feel like I put a lot of myself into it!

**Q: What do you plan to do moving forward?**
A: Apart from putting out songs, I'd also want to start posting more videos!

### Dreams
1. Hold a solo live concert on a big stage
2. Get a statue figure
3. Collab with an aquarium

---

## Theme & Motif Summary

- **Archetype:** Secret agent / marine creature / mysterious intern
- **Secret Society role:** Fixer and cleaner — handles elimination and cleanup for holoX
- **Marine biology motif:** Orca/killer whale (black-and-white coloring, fin-like hair, aquatic accessories)
- **Fishing motif:** Fan mark 🎣, fan name "Handlers" (fishing/handler terminology), fish emoji in chat
- **Name etymology:**
  - 沙花叉 (Sakamata) — also the name of a type of fish (sakamata = killer whale / orca in some Japanese dialects)
  - クロヱ (Chloe/Kuroe) — "kuro" (black) fitting the dark/mysterious theme
- **Personality paradox:** Officially described as "calm and composed" but known to be guarded and not upfront; her streaming persona often shows a much more energetic, emotional, and chaotic side

---

## Social Links

- **YouTube:** https://www.youtube.com/channel/UCIBY1ollUsauvVi4hW4cumw
- **X (Twitter):** https://twitter.com/sakamatachloe
- **TikTok:** https://www.tiktok.com/@sakamatachloe_hololive
- **Song playlist:** https://www.youtube.com/watch?v=rFbJlErkcGw&list=PLi6TWx3pTf1fVyYNxUCKNHmmssZjKnK9K&index=1
- **MMD model:** https://3d.nicovideo.jp/works/td84836

---

## Source URLs

1. **Official Talent Page (EN)** — https://hololive.hololivepro.com/en/talents/sakamata-chloe/
2. **Official Talent Page (JP)** — https://hololive.hololivepro.com/talents/sakamata-chloe/
3. **holoX Debut Announcement** — https://hololive.hololivepro.com/en/news/20211126-1-41/
4. **Illustrator Twitter (Parsley)** — https://twitter.com/Parsley_F
5. **Live2D Modeler (jenny / IrisLight)** — https://twitter.com/jennya_IRIS
6. **Hololive Wiki (Miraheze)** — https://hololive.wiki/wiki/Sakamata_Chloe
7. **Fandom Wiki** — https://hololivevtuber.fandom.com/wiki/Sakamata_Chloe
8. **Seesaawiki (非公式wiki)** — https://seesaawiki.jp/hololivetv/d/%BA%BB%B2%D6%BA%B5%A5%AF%A5%ED%A5%F1
