# 沙花叉クロエ (Sakamata Chloe) — 同人遊戲設定參考報告

## 基本資料

| 項目 | 內容 |
|------|------|
| **所屬** | Hololive 6期生「Secret Society holoX」 |
| **角色定位** | holoX 的實習生・處理者（Fixer / Cleaner） |
| **出道日** | 2021年11月29日 |
| **生日** | 5月18日 |
| **身高** | 148 cm |
| **繪師** | パセリ (Parsley) |
| **粉絲名稱** | Handlers / Zookeepers（飼育員さん） |
| **粉絲記號** | 🎣 |
| **社群名稱** | Orcart（オーカート） |
| **Live2D 建模** | 入江燈 (jenny / IrisLight) |
| **3D 建模** | ユウコテツ (Yukotetsu) |
| **愛稱** | シャチけん (Shachiken) |
| **官方標語** | "Chomp, chomp, chooomp!" |
| **YouTube** | https://www.youtube.com/channel/UCIBY1ollUsauvVi4hW4cumw |
| **X (Twitter)** | https://twitter.com/sakamatachloe |

---

## 角色設計與主題

### 視覺設計
- **虎鯨/殺人鯨 (Orca) 主題** — 黑白藍配色，頭髮後方有鰭狀突起，整體造型充滿海洋生物感
- **祕密特務風格** — 帽子、手套、貼身戰術服裝、工具腰包
- **不對稱腿飾** — 一腿過膝襪、一腿膝上襪
- **顏色方案** — 黑、白、藍/青色（背景主色 #ab0e0c/#cf4c4a 紅色系，#921a1b）

### 核心主題
- **海洋生物**：虎鯨（日語「サカマタ」即為虎鯨的一種方言稱呼）
- **祕密結社**：holoX 的處理者，負責消滅證據、排除障礙
- **釣魚**：粉絲記號🎣、粉絲名「Handlers」(飼育員/處理者)
- **反差萌**：表面上冷靜沉著，內心卻不擅長表達真實情感

---

## 官方設定（同人遊戲劇情基礎）

**Secret Society holoX 的實習生兼處理者。**
> 「もぐもぐ、もぐもぐ〜ん！」
> 冷靜沉著，執行命令時眉頭都不皺一下。
> 非常喜歡音樂，隨時隨地都在聽音樂。
> 雖然極力否認，但她似乎給人一種防備心強、不善表達真實想法的印象，偶爾才會讓人窺見她的真心。

### 可發展的劇情元素：
1. **實習生身份** — holoX 中地位最低但不可或缺的成員，適合描寫成長與認同
2. **處理者職責** — 暗含暴力/陰暗面的設定，可作為嚴肅劇情線索
3. **音樂熱情** — 作曲、作詞、歌唱、發聲訓練，夢想是舉辦個人演唱會
4. **口是心非** — ツンデレ（傲嬌）性格，官方認證的「否認自己被說防備心強」

---

## 性格特質（角色塑造參考）

| 特質 | 說明 | 遊戲應用建議 |
|------|------|------------|
| **冷靜專業** | 表面沉著，執行任務可靠 | 平時的默認狀態 |
| **傲嬌/口是心非** | 被說中真心話會極力否認 | 好感度事件核心 |
| **愛睡覺** | 不直播時都在睡覺 | 慵懶日常場景 |
| **音樂狂熱** | 作曲作詞歌唱全部自己來 | 特殊能力/技能樹 |
| **混沌/情緒化** | 直播中常展現反差激烈情緒 | 劇情高潮表現 |
| **幽默感** | 突發怪聲、自嘲 | 日常互動笑點 |
| **ぼんやり (PON)** | 容易走神、發呆 | 可愛失誤事件 |
| **遊戲大師** | 擅長桌遊、黑白棋等策略遊戲 | 智力型角色定位 |
| **勝負心強** | 討厭輸 | 對戰/競爭劇情 |

---

## 口頭禪與直播慣用語（台詞素材）

| 場合 | 台詞 |
|------|------|
| 開場 | "Chomp, chomp, chomp! It's time to eat!"（もぐもぐ、もぐもぐ〜ん！） |
| 結尾 | "Thanks for the food."（ごちそうさまでした） |
| 等待室符號 | `＜＞＜`（魚的ASCII藝術） |
| 稱呼粉絲 | "Handler."（飼育員さん） |
| 自我介紹 | "Chomp, chomp, chooomp!" |

---

## 主要關係網絡（NPC/角色關係設定）

### holoX 同期生（主線角色群）
| 成員 | 角色定位 | 關係動態 |
|------|---------|---------|
| **拉普拉斯·達克尼斯** (La+ Darknesss) | holoX 首領 | 上下級關係，首領對實習生的互動 |
| **鷹嶺琉衣** (Takane Lui) | holoX 秘書 | 同事，秘書與處理者的工作搭檔 |
| **博衣小夜璃** (Hakui Koyori) | holoX 研究開發 | **最親近的同期**，常合作、平衡彼此能量 |
| **風真伊呂波** (Kazama Iroha) | holoX 守衛隊長 | 同事關係 |

### 重要配對（可發展的特別劇情）
| 配對名 | 對象 | 特點 |
|--------|------|------|
| **Kanaken (かなけん)** | 天音彼方 (Amane Kanata, 4期生) | 玩闹互補，合作翻唱《Rabbit Hole》 |
| **Shioshachi (塩シャチ)** | 紫咲詩音 (Murasaki Shion, 2期生) | 性格對比組合；引退後仍合作翻唱〈チューリングラブ〉(2025/3/31) |

### 所屬團體
- **UMISEA** — 海洋主題跨期生團體
- **Magical Girl holoWitches!** — 魔法少女跨期團體（成員：Kanata, Miko, Luna, Marine, Shion, Chloe）

---

## 里程碑事件（劇情章節參考）

| 時間 | 事件 | 遊戲化建議 |
|------|------|-----------|
| 2021.11.29 | 出道直播，首日達20萬訂閱 | 序章/開場 |
| 2021.12.10 | 12天內達60萬訂閱 | 早期成長劇情 |
| 2022.06.13 | 3D模型披露 | 角色覺醒/進化事件 |
| 2023.02.18 | 達成100萬訂閱（耐久歌回中達成） | 關鍵成就解鎖 |
| 2022.10 | holoX MEETing 漫畫連載開始（Jump+） | 支線劇情 |
| 2024.11.29 | 宣布 streaming 活動結束（原因：健康問題與過勞） | 終章/劇情轉折 |
| 2025.01.26 | 最終3D Live直播 | 結局 |
| 2025.03.31 | 引退後首次回歸 — 與紫咲詩音合作翻唱〈チューリングラブ〉 | 特別回歸/番外篇 |

---

## 音樂作品（可作為遊戲BGM/特殊演出）

### 原創歌曲
- 《人生リセットボタンぽちーw》（出道原創曲）
- 《Paralyze》
- 《Moodoku》
- 《Mazaru Nibiiro》
- 專輯《Secret ORDER》

### holoX 團體曲
- 《Jouya Repaint》
- 《Cho Yume Dodekkai!》
- 《Kyouen Solidarity ~SHAKE★NABE~》
- 《Our Bright Parade》

### 知名翻唱
- 《Rabbit Hole》(DECO*27) — 與天音彼方合唱
- 《Monster》(YOASOBI) — 多人合作
- 《Animal》(DECO*27) — 個人翻唱
- 《チューリングラブ》 — 與紫咲詩音合唱（引退後回歸作，2025/3/31）

### 音樂喜好（角色設定參考）
- 海外アーティスト：Sia、Bruno Mars
- 邦樂：ヨルシカ、ZUTOMAYO
- 喜歡的音樂類型：J-Pop、Rock、電子搖滾

---

## 同人遊戲設定靈感摘要

### 核心衝突點
1. **身份矛盾** — 實習生 vs 最危險的處理者
2. **表裡不一** — 冷靜專業 vs 內心脆弱傲嬌
3. **職責與夢想** — holoX 的 dirty work vs 想開個人演唱會的夢想
4. **離別與傳承** — 2025年活動結束，成為首位 hololive「Affiliate」成員

### 可採用的遊戲類型建議
- **視覺小說/AVG** — 豐富的官方設定與關係網絡適合劇情向作品
- **音樂節奏遊戲** — 角色對音樂的熱情與大量原創曲
- **冒險/角色扮演** — holoX 秘密結社設定適合任務導向玩法

### 視覺風格關鍵字
虎鯨、黑白藍、海洋、特務、軍帽、鰭狀頭髮、不對稱服裝、🎣、`＜＞＜`

### 音樂風格方向
J-Pop / Rock，參考《Moodoku》《人生リセットボタンぽちーw》的電子搖滾風格

---

## 資料來源
1. Hololive 官方檔案 (EN): https://hololive.hololivepro.com/en/talents/sakamata-chloe/
2. Hololive 官方檔案 (JP): https://hololive.hololivepro.com/talents/sakamata-chloe/
3. holoX 出道公告: https://hololive.hololivepro.com/en/news/20211126-1-41/
4. COVER Corp 官方說明（活動結束）: https://note.cover-corp.com/n/ne3a8b7a553c0
5. Hololive Wiki: https://hololive.wiki/wiki/Sakamata_Chloe
6. ファンダム Wiki: https://hololivevtuber.fandom.com/wiki/Sakamata_Chloe
7. 非公式wiki: https://seesaawiki.jp/hololivetv/d/%BA%BB%B2%D6%BA%B5%A5%AF%A5%ED%A5%F1
8. YouTube 頻道: https://www.youtube.com/@SakamataChloe
